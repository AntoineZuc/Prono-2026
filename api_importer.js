...
const WORKER_URL = 'https://tiny-feather-9126.antoinezuccari.workers.dev';

async function fdorgFetch(path, apiKey){
  const url=`${WORKER_URL}?apikey=${encodeURIComponent(apiKey)}&path=${encodeURIComponent(path)}`;
...
const FDORG_COMPETITIONS = {
  'CL':  {label:'Ligue des Champions (UEFA)',  stages:['LEAGUE_PHASE','GROUP_STAGE'], type:'clubs'},
  'EL':  {label:'Europa League (UEFA)',         stages:['LEAGUE_PHASE','GROUP_STAGE'], type:'clubs'},
  'EC':  {label:'Euro (UEFA)',                  stages:['GROUP_STAGE','LEAGUE_PHASE'], type:'nations'},
  'WC':  {label:'Coupe du Monde (FIFA)',        stages:['GROUP_STAGE','LEAGUE_PHASE'], type:'nations'},
  'PL':  {label:'Premier League',               stages:['REGULAR_SEASON'],             type:'clubs'},
  'PD':  {label:'La Liga',                      stages:['REGULAR_SEASON'],             type:'clubs'},
  'BL1': {label:'Bundesliga',                   stages:['REGULAR_SEASON'],             type:'clubs'},
  'SA':  {label:'Serie A',                      stages:['REGULAR_SEASON'],             type:'clubs'},
  'FL1': {label:'Ligue 1',                      stages:['REGULAR_SEASON'],             type:'clubs'},
};

// Compétitions qui utilisent des pays (drapeaux) vs clubs (logos)
function isNationsComp(code){ return FDORG_COMPETITIONS[code]?.type==='nations'; }

// Clé API football-data.org — sauvegardée dans Firebase /apiKeys/footballData
// Clé par défaut (peut être remplacée depuis l'interface)
const DEFAULT_API_KEY='ca8c90bab9334250b7a4964462793313';
...
async function fdorgGetApiKey(){
  try{
    ...
    if(!r.ok)return DEFAULT_API_KEY;
    const d=await r.json();
    return d||DEFAULT_API_KEY;
...
async function fdorgGetMatches(apiKey, competitionCode, season){
  return await fdorgFetch(`/competitions/${competitionCode}/matches?season=${season}`, apiKey);
}

async function fdorgGetTeams(apiKey, competitionCode, season){
  return await fdorgFetch(`/competitions/${competitionCode}/teams?season=${season}`, apiKey);
}

// Convertir un match football-data → format interne du site
function fdorgConvertMatch(m, teamsMap, idx, compCode){
  ...
  const isNations=isNationsComp(compCode);
  const homeId=m.homeTeam?.id;
  const awayId=m.awayTeam?.id;
  const homeInfo=teamsMap[homeId]||{};
  const awayInfo=teamsMap[awayId]||{};

  // Pour les nations : utiliser le code pays de teams_db si dispo
  // Pour les clubs : utiliser l'ID fdorg comme code unique (évite les conflits TLA)
  const homeCode=isNations
    ?(homeInfo.dbCode||m.homeTeam?.tla||String(homeId))
    :String(homeId); // ID numérique fdorg comme clé unique pour les clubs
  const awayCode=isNations
    ?(awayInfo.dbCode||m.awayTeam?.tla||String(awayId))
    :String(awayId);

  const homeName=homeInfo.name||m.homeTeam?.shortName||m.homeTeam?.name||'?';
  const awayName=awayInfo.name||m.awayTeam?.shortName||m.awayTeam?.name||'?';
  const homeCrest=homeInfo.crest||m.homeTeam?.crest||null;
  const awayCrest=awayInfo.crest||m.awayTeam?.crest||null;
  const homeFlag=homeInfo.iso||null; // pour les nations seulement

  // Groupe (nations) ou journée (clubs)
  const wcGroup=isNations?(m.group||m.stage||'?'):`J${m.matchday||'?'}`;
  const matchday=m.matchday;

  return{
    id:`${date}-${homeCode}-${awayCode}`.toLowerCase().replace(/[^a-z0-9]+/g,'-'),
    matchNo:idx+1,
    date,time,wcGroup,matchday,
    teamA:homeCode,teamB:awayCode,
    teamAName:homeName,teamBName:awayName,
    teamACrest:homeCrest,teamBCrest:awayCrest,
    teamAFlag:homeFlag,teamBFlag:awayInfo.iso||null,
    isClubs:!isNations,
    scoreA:null,scoreB:null,
    fdorgId:m.id,fdorgHomeId:homeId,fdorgAwayId:awayId,
    status:m.status,
  };
}
  ...
  document.getElementById('fdImportBtn')?.addEventListener('click',async()=>{
    ...
    try{
      ...
      try{
        ...
        const nations=isNationsComp(code);
        (teamsData.teams||[]).forEach(t=>{
          const name=t.shortName||t.name||'?';
          const tla=(t.tla||'').toUpperCase();
          if(nations){
            // Matcher avec teams_db.js par nom ou TLA — JAMAIS par TLA seul (risque ESP=Espanyol)
            let dbCode=null;
            if(typeof findTeamByName!=='undefined'){dbCode=findTeamByName(name)||findTeamByName(t.name||'');}
            if(!dbCode&&typeof TEAMS_DB!=='undefined'&&TEAMS_DB[tla]){
              // Vérifier que le nom correspond (évite ESP → Espanyol)
              const dbName=TEAMS_DB[tla][0].toLowerCase();
              if(name.toLowerCase().includes(dbName.slice(0,4))||dbName.includes(name.toLowerCase().slice(0,4)))
                dbCode=tla;
            }
            const iso=dbCode&&typeof TEAMS_DB!=='undefined'&&TEAMS_DB[dbCode]?TEAMS_DB[dbCode][1]:'xx';
            teamsMap[t.id]={dbCode,name,iso,crest:t.crest||null};
          } else {
            // Clubs : stocker l'ID numérique + crest (logo) + nom
            teamsMap[t.id]={name,tla,crest:t.crest||null,iso:null,dbCode:null};
          }
        });
      }catch(e){
      ...
      const converted=groupMatches.map((m,i)=>fdorgConvertMatch(m,teamsMap,i,code));
...
function renderFdPreview(matches, teamsMap, compCode, season, apiKey, compInfo, onImported){
  ...
  document.getElementById('fdConfirmImport')?.addEventListener('click',()=>{
    if(onImported)onImported({
      ...
      isClubs:!isNationsComp(compCode),
    });
  });
}
