...
async function fdorgFetch(path, apiKey){
  // Via proxy CORS — la clé passe en query param pour éviter les problèmes de headers
  const targetUrl=FDORG_URL+path+'&X-Auth-Token='+encodeURIComponent(apiKey);
  try{
    const r=await fetch(CORS_PROXY+encodeURIComponent(FDORG_URL+path),{
      headers:{'x-auth-token':apiKey,'X-Requested-With':'XMLHttpRequest'}
    });
    if(r.ok)return await r.json();
  }catch(e){}
  // Fallback : injection dans l'URL (certains proxies le supportent)
  try{
    const r2=await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(FDORG_URL+path+'?apikey='+apiKey)}`);
    if(r2.ok){const d=await r2.json();return JSON.parse(d.contents);}
  }catch(e){}
  throw new Error('Impossible de contacter l\'API — vérifie ta clé et ta connexion.');
}
