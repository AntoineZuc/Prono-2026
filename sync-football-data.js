...
async function fdFetch(comp, season) {
  // Fenêtre de quelques jours autour d'aujourd'hui seulement — inutile de
  // retélécharger toute la saison (300+ matchs) à chaque passage, juste ce
  // qui a pu changer récemment.
  const now = new Date();
  const dateFrom = new Date(now.getTime() - 3*86400000).toISOString().slice(0,10);
  const dateTo = new Date(now.getTime() + 3*86400000).toISOString().slice(0,10);
  const r = await fetch(`https://api.football-data.org/v4/competitions/${comp}/matches?season=${season}&dateFrom=${dateFrom}&dateTo=${dateTo}`, {
...
async function syncSection(section) {
  ...
  data.matches.forEach((m) => {
    ...
    // Scores (couvre aussi les matchs EN COURS, pas juste les terminés —
    // le champ score.fullTime est mis à jour en direct par football-data.org
    // pendant le match, avant de devenir définitif une fois FINISHED)
    if (m.score?.fullTime?.home != null) {
      const s = { scoreA: m.score.fullTime.home, scoreB: m.score.fullTime.away, status: m.status };
      if (!existingScores[id] || existingScores[id].scoreA !== s.scoreA || existingScores[id].scoreB !== s.scoreB || existingScores[id].status !== s.status) {
        newScores[id] = s;
        scoresChanged = true;
      }
    }
